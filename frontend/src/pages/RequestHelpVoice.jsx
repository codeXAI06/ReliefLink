...
function RequestHelp() {
  ...
  const handleVoiceTranscript = async ({ text, duration, language }) => {
    ...
    try {
      ...
      const result = await processVoiceInput({
        ...
        detectedLanguage: language?.split('-')[0], // e.g., 'hi-IN' -> 'hi'
        preferredLanguage: currentLanguage,
      });
      
      if (result.success && result.extracted_request) {
        ...
        if (result.errors?.length > 0) {
          setProcessingError(result.fallback_reason || result.errors[0]);
        }
      ...
      setProcessingError(t('voiceProcessingError') || 'Could not process voice. Please type your request.');
  ...
  // Handle text input processing (for fallback)
  const handleTextExtract = async () => {
    ...
    try {
      ...
      // Silent fail - user can continue with manual input
    } finally {
      setIsProcessingVoice(false);
    }
  };
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6">
              ...
              <div className="mt-4 text-center">
                <div className="inline-flex items-center px-4 py-2 bg-white rounded-full shadow">
                  ...
                  <span className="text-gray-600">
                    {t('processingVoice') || 'Processing your request...'}
                  </span>
                </div>
              </div>
            )}
      ...
      {/* Manual Form (Text Mode or after AI confirmation) */}
      {(inputMode === INPUT_MODES.TEXT || !showAIForm) && !showAIForm && (
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Description (for text mode) */}
          {inputMode === INPUT_MODES.TEXT && (
            <div>
          ...
          {/* Description (if not in text mode - show after AI form) */}
      ...
      {/* After AI confirmation - show remaining form */}
      {!showAIForm && extractedData && (
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Location */}
          <div id="location-section">
